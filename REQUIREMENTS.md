...
**当前缺失**：
- LangGraph 编排图 `build_orchestrator()` 仍为占位（返回 None）
- 前端资源卡片尚未对接后端 `/api/resource/generate`
- 无法端到端自动生成资源

---

### F3. 个性化学习路径规划和资源推送 🚧 框架完成，待接入
...
| 精准推送（文档/视频/题库/实操） | ⏳ 待实现 | — | 第 4 周实现推送逻辑 |
| 前端路径可视化 | 🚧 部分实现 | `app.py` 学习路径 Tab | 展示了静态路径，未动态加载 |
